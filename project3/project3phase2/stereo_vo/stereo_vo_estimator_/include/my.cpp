...
bool Estimator::inputImage(ros::Time time_stamp, const cv::Mat& _img, const cv::Mat& _img1) {
  ...
  c_R_k.setIdentity(); // this is current cam to keypoint, the comment in estimator.h is wrong
  c_t_k.setZero(); // this is current cam to keypoint, the comment in estimator.h is wrong 
  ...
  ud_right_pts_2d = Estimator::undistortedPts(left_pts_2d, m_camera[0]);
  Estimator::generate3dPoints(ud_left_pts_2d, ud_right_pts_2d, cur_pts_3d, cur_pts_2d);
  ...
  // cv::solvePnP(cur_frame.xyz, cur_frame.uv, fake_K, D, rvec, t);
  // cv::Rodrigues(rvec, r);
  // Matrix3d R_ref;
  // Vector3d T_ref;
  // for (int i = 0; i < 3; i++){
...
bool Estimator::estimateTBetweenFrames(vector<cv::Point3f>& key_pts_3d,
  ...
  cv::Mat fake_K = cv::Mat::eye(3, 3, CV_64F);
  cv::solvePnP(key_pts_3d, ud_cur_pts_2d, fake_K, D, rvec, t_mat);
  cv::Rodrigues(rvec, r);
  for (int i = 0; i < 3; i++){
...
bool Estimator::trackFeatureLeftRight(const cv::Mat& _img, const cv::Mat& _img1,
  ...
  vector<uchar> status_list;
  vector<float> error_list;
  cv::calcOpticalFlowPyrLK(_img, _img1, left_pts, right_pts, status_list, error_list, cv::Size(21,21), 3);
...
vector<cv::Point2f> Estimator::undistortedPts(vector<cv::Point2f>& pts, camodocal::CameraPtr cam) {
  ...
  // for (int i = 0; i < cur_pts_3d.size(); i++) {
  //     std::cout << (cur_pts_3d.at(i)) << endl;
  // }
  
  // for (int i = 0; i < cur_pts_2d.size(); i++) {
  //     std::cout << (cur_pts_2d.at(i)) << endl;
  // }
  // cout << "===================" << endl;