...
for j = 1 : M-1
    A_continuity((1+4*(j-1)):(4+4*(j-1)),(1+8*(j-1)):(16+8*(j-1)))= ...
        ...
        0, time_interval(j)^0,2*time_interval(j)^0, 6*time_interval(j)^0,24*time_interval(j)^1,60*time_interval(j)^2,120*time_interval(j)^3,210*time_interval(j)^4,0,0,0,-6,0,0,0,0;];
end
